...
void ExampleApp::DrawScene()
{
    ...
    // obj 그리기
    worldViewProj = XMLoadFloat4x4(&mObjWorld) * viewProj;
    mfxWorldViewProj->SetMatrix(reinterpret_cast<float*>(&worldViewProj));
    for (UINT p = 0; p < techDesc.Passes; ++p)
    {
        mTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
        md3dImmediateContext->DrawIndexed(mObjIndexCount, mObjIndexOffset, mObjVertexOffset);
    }
...
void ExampleApp::BuildGeometryBuffers()
{
   /* GeometryGenerator::MeshData grid;
    ...
    ObjGenerator objGen;
    objGen.CreateObj("D:/GitHub/CppStudy/Sting-Sword-lowpoly.obj", obj);

    mObjVertexOffset = 0;
    mObjIndexCount = obj.Indices.size();
    mObjIndexOffset = 0;
    /*mGridVertexOffset = 0;
    ...
    UINT totalVertexCount = obj.Vertices.size();
